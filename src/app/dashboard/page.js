...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadCitas = async () => {
      try {
        const response = await fetch(`/api/citas?usuario_id=${usuarioGuardado.id}`, {
          cache: 'no-store',
        });
        const result = await response.json();

        if (Array.isArray(result)) setCitas(result);
        else if (result.data && Array.isArray(result.data)) setCitas(result.data);
        else setCitas([]);
      } catch (error) {
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      setCitas((prev) =>
        prev.map((c) => (c.id === citaParaCancelar.id ? { ...c, estado: 'Cancelada' } : c))
      );
  ...
  const handleEditAppointment = (citaAEditar) => {
    router.push(`/dashboard/agendar?edit=${citaAEditar.id}`);
  };
    ...
    <div className="min-h-screen bg-[#F7F8FA]">
      ...
      <main className="max-w-7xl mx-auto py-8 px-6">
        ...
        <div className="space-y-4">
          {(activeTab === 'proximas' ? proximasCitas : historialCitas).map((cita) => (
            <CitaCard
              key={cita.id}
              cita={cita}
