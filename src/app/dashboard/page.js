...
// CORRECCIÓN: Definimos los tipos esperados para ConfirmationModal
...
// CORRECCIÓN: Definimos los tipos esperados para StatCard
StatCard.propTypes = {
  title: PropTypes.string.isRequired,
  value: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
  icon: PropTypes.elementType.isRequired, // Lucide icons son componentes
          ...
          <div className="flex gap-6">
            <button
              ...
              title={
                canEdit
                  ? "Editar cita"
                  : "Solo se puede editar con >7 días de anticipación"
              }
            >
              <SquarePen size={18} />
            </button>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const inicioSesion = Number.parseInt(
      localStorage.getItem("inicioSesion"),
      10
    );
    if (!inicioSesion) {
      router.push("/");
  ...
  const historial = citas.filter(
    (c) =>
      c.estado === "Cancelada" ||
      (c.fecha < todayISO && c.estado !== "Cancelada")
  );
